...
- `!12/25` (no space) — a **field** exclusion: the 25th of any month *except* December.
...
isnow answers "does it hold now?" `next` and `prev` are *derived* from that and are unbounded. There is no "run exactly 10 times": `COUNT` needs an anchor the language does not yet have. If you need a fixed, counted series, reach for iCalendar and gate it with isnow.
